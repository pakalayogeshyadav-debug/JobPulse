...
def test_change_detector_new_and_updated():
    ...
    detector._compute_hashes = MagicMock(return_value={
        "job1": "h1",
        "job2": "h2",
        "job3": "h3"
    })
...
def test_change_detector_persist_hashes():
    ...
    detector.persist_hashes([
        RowChange(source_job_id="job1", source_name="test_src", classification=ChangeClassification.NEW, new_hash="h1"),
        RowChange(source_job_id="job2", source_name="test_src", classification=ChangeClassification.UPDATED, new_hash="h2", old_hash="h1"),
    ])
