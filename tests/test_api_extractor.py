...
def test_api_extractor_extract_success(mock_fetch_page):
    ...
    extractor = DummyApiExtractor("dummy", "http://dummy", "secret", max_pages=3, request_delay_seconds=0.0)
...
def test_api_extractor_extract_max_pages(mock_fetch_page):
    ...
    extractor = DummyApiExtractor("dummy", "http://dummy", "secret", max_pages=2, request_delay_seconds=0.0)
    df = extractor.extract()
    assert len(df) == 2
